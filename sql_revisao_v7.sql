-- ============================================================
--  ConstrutorRH — Script SQL Definitivo v7
...
-- status: adicionar 'recusado' se ainda não existir
DO $$
BEGIN
  -- Verifica e remove constraint antiga para recriar com novo valor
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints tc
    JOIN information_schema.check_constraints cc USING (constraint_name)
    WHERE tc.table_name = 'adiantamentos'
      AND cc.check_clause LIKE '%pendente%'
      AND cc.check_clause NOT LIKE '%recusado%'
  ) THEN
    ALTER TABLE public.adiantamentos DROP CONSTRAINT IF EXISTS adiantamentos_status_check;
  END IF;
END $$;
...
-- ─────────────────────────────────────────────────────────────
-- 8. obras — garantir status 'em_andamento'
-- ─────────────────────────────────────────────────────────────
-- O sistema usa status 'em_andamento' para obras ativas
-- Se sua tabela tiver status='ativo', execute o UPDATE abaixo:
-- UPDATE public.obras SET status = 'em_andamento' WHERE status = 'ativo';

-- Adicionar 'em_andamento' ao check se necessário
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints tc
    ...
    WHERE tc.table_name = 'obras'
      AND cc.check_clause LIKE '%ativo%'
...
-- ─────────────────────────────────────────────────────────────
-- 9. configuracoes — campos novos de empresa
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 11. Storage — bucket "documentos" + políticas via RLS
-- ─────────────────────────────────────────────────────────────
