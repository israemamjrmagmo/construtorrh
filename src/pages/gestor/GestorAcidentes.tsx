...
interface AcidenteRow {
  ...
  cat_emitida: boolean; status: string; created_at: string
}
    ...
    try {
      const [{ data }, { data: ultimo }] = await Promise.all([
        supabase.from('acidentes')
          .select(`
            id, data_ocorrencia, hora_acidente, tipo, descricao,
            gravidade, cat_emitida, status, created_at,
            colaboradores(nome, chapa, funcoes(nome), obras(nome))
          `)
          .gte('data_ocorrencia', dtIni)
      ...
      if (ultimo && ultimo.length > 0) {
        const ultData = new Date(ultimo[0].data_ocorrencia + 'T12:00')
      ...
      setRows((data ?? []).map((r: any) => ({
        id:                r.id,
        data_acidente:     r.data_ocorrencia,
        ...
        status:            r.status   ?? 'em_investigacao',
              ...
              {rowsFiltrados.length === 0 ? (
                <tr>
                  <td colSpan={7} style={{ textAlign: 'center', padding: 32, color: '#94a3b8' }}>
                    {resumo.total === 0
                      ? '🏆 Nenhum acidente no período! Continue com as boas práticas.'
                ...
                return (
                  <tr key={r.id} style={{ borderTop: '1px solid #f1f5f9', background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                    ...
                    <td style={{ padding: '10px 14px', color: '#374151' }}>
                      ...
                      {r.descricao && (
                        <div style={{ fontSize: 10, color: '#94a3b8', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {r.descricao}
                        </div>
                      )}
                    </td>
                    <td style={{ padding: '10px 14px', textAlign: 'center' }}>
                      <span style={{
                        ...
                        background: gc.bg, color: gc.cor, fontWeight: 700, fontSize: 11,
                      }}>
                        {gc.label}
                      </span>
                    </td>
