...
// ── Data isolada — também não precisa atualizar o layout ─────────────────────
const DataStrip = memo(function DataStrip() {
  const data = useMemo(
    () => new Date().toLocaleDateString('pt-BR', { weekday: 'short', day: '2-digit', month: 'short' }),
    []
  )
  return (
    <span style={{ fontSize: 10, color: '#94a3b8', fontWeight: 500, whiteSpace: 'nowrap', flexShrink: 0 }}>
      {data}
    </span>
  )
})

// ── Conteúdo da Sidebar — memoizado para não re-renderizar com o relógio ─────
...
export default function PortalLayout({ children }: PortalLayoutProps) {
  ...
  // Sessão estável — só lida uma vez
  const user = useMemo(() => getPortalSession(), [])

  const [online,      setOnline]      = useState(navigator.onLine)
  const [sidebarOpen, setSidebarOpen] = useState(false)

  // Status de conexão (não causa re-render no intervalo do relógio)
  useEffect(() => {
  ...
  const sair = useCallback(() => { clearPortalSession(); nav('/portal') }, [nav])
  ...
  // Callback estável para fechar o drawer
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#f0f2f5', fontFamily: "'Inter','Segoe UI',sans-serif", display: 'flex', flexDirection: 'column' }}>
      <style>{`
        ...
        .sidebar-link:hover { background: #f1f5f9 !important; }
      ...
      <div className="portal-topbar">
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 2 }}>
              <div style={{
                ...
                boxShadow: online
                  ? '0 0 0 2px rgba(74,222,128,0.40)'
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
          ...
          {/* ✅ Relógio em componente isolado — não re-renderiza o layout */}
          <RelogioTopbar />
      ...
      <div className="portal-body">
        ...
        <aside className="portal-sidebar">
          <SidebarContent location={location} onClick={closeSidebar} />
        </aside>

        {/* SIDEBAR DRAWER mobile */}
        <div className={`portal-sidebar-overlay${sidebarOpen ? ' open' : ''}`} onClick={closeSidebar} />
