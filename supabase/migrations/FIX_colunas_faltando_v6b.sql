-- ═══════════════════════════════════════════════════════════════════════════════
-- FIX v2: Continuar correção após erro de coluna duplicada
-- Execute no SQL Editor do Supabase
-- ═══════════════════════════════════════════════════════════════════════════════

-- ─── ACIDENTES ───────────────────────────────────────────────────────────────
-- Remove coluna hora_ocorrencia se existir (hora_acidente já foi criada)
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns
             WHERE table_name='acidentes' AND column_name='hora_ocorrencia') THEN
    ALTER TABLE public.acidentes DROP COLUMN hora_ocorrencia;
  END IF;
END $$;

-- Renomear data_acidente → data_ocorrencia (só se ainda existir)
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns
             WHERE table_name='acidentes' AND column_name='data_acidente') THEN
    ALTER TABLE public.acidentes RENAME COLUMN data_acidente TO data_ocorrencia;
  END IF;
END $$;

-- Adiciona colunas restantes que podem faltar
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns
             WHERE table_name='atestados' AND column_name='tipo_afastamento') THEN
...
SELECT
  (SELECT string_agg(column_name, ', ' ORDER BY column_name)
   FROM information_schema.columns WHERE table_name = 'acidentes') AS acidentes,
  (SELECT string_agg(column_name, ', ' ORDER BY column_name)
   FROM information_schema.columns WHERE table_name = 'advertencias') AS advertencias,
  (SELECT string_agg(column_name, ', ' ORDER BY column_name)
   FROM information_schema.columns WHERE table_name = 'atestados') AS atestados;
